...
"use client";
// Adapted from reactbits "Lanyard". A draggable ID-card hanging on a physics
// rope. The card face texture is replaced with our own Pacome badge so it
// doesn't show the original demo design.
...
const BADGE_PNG = "/lanyard/badge.png";
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: { maxSpeed?: number; minSpeed?: number; isMobile?: boolean }) {
  ...
  const badge = useTexture(BADGE_PNG);
  ...
  // Swap the card's front texture for our own badge.
  useEffect(() => {
    if (badge) {
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? "kinematicPosition" : "dynamic"}>
          ...
          >
            <mesh geometry={nodes.card.geometry}>
              {/* Our custom badge texture instead of materials.base.map */}
              <meshPhysicalMaterial map={badge} map-anisotropy={16} clearcoat={isMobile ? 0 : 1} clearcoatRoughness={0.15} roughness={0.85} metalness={0.4} />
            </mesh>
