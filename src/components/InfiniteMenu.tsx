"use client";
// Source: reactbits.dev / InfiniteMenu (icosahedron-sphere disc menu with arcball control)
// Adapted to TypeScript-friendly JS with the same behavior. Stretches images
// along the rotation axis on motion — perfect for a portfolio carousel.
...
function createProgram(gl: WebGL2RenderingContext, sources: [string, string], attribLocations?: Record<string, number>) {
  const program = gl.createProgram()!;
  [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((type, ndx) => {
    const sh = createShader(gl, type, sources[ndx]); if (sh) gl.attachShader(program, sh);
  });
...
class InfiniteGridMenu {
  ...
  SPHERE_RADIUS = 2;
  _time = 0; _deltaTime = 0; _deltaFrames = 0; _frames = 0;
  camera = {
  ...
  constructor(canvas: HTMLCanvasElement, items: MenuItem[], onActiveItemChange: (i: number) => void, onMovementChange: (m: boolean) => void, onInit: ((s: InfiniteGridMenu) => void) | null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  _init(onInit: ((s: InfiniteGridMenu) => void) | null) {
  ...
  _initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // fit image to cell (object-fit cover-style)
        ctx.fillStyle = "#000";
        ctx.fillRect(x, y, cellSize, cellSize);
        const ar = img.width / img.height;
        let dw = cellSize, dh = cellSize;
        if (ar > 1) { dh = cellSize; dw = cellSize * ar; }
        else { dw = cellSize; dh = cellSize / ar; }
...
export default function InfiniteMenu({ items, scale = 1.0, onActiveItemChange, onMovingChange }: Props) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      const it = items[index % items.length] || null;
      setActiveItem(it);
      onActiveItemChange?.(it);
    };
    const handleMoving = (m: boolean) => {
      setIsMoving(m);
      onMovingChange?.(m);
    };
    const sketch = new InfiniteGridMenu(
      canvas,
      items.length ? items : [{ image: "", link: "", title: "", description: "" }],
      handleActiveItem,
      handleMoving,
      (sk) => sk.run(),
      scale
  ...
  return (
    <div className="infinite-menu-wrapper">
      ...
      {/* Title / description / action overlay (only render on >= 1500px screens via CSS) */}
      {activeItem && (
        <>
